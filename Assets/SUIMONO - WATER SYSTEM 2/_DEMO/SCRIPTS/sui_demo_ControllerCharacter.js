...
if (isActive){
	...
	// SET MOVEMENT SPEEDS
	//---------------------------------
	if (isUnderWater) moveSideways = 0.00;
	...
	if (isControllable){
		...
		if (!orbitView){
			...
			if (moveForward != 0.0){
				...
				if (isInWater){
					if (isAtSurface){
						cameraTarget.GetComponent.<Rigidbody>().velocity = Vector3(0,0,0);
					}
					
					xMove = Mathf.Lerp(xMove,useSpeed,Time.deltaTime);
					cameraTarget.eulerAngles.x += Mathf.Lerp(0.0,-130.0*moveVert,Time.deltaTime*(Mathf.Abs(xMove*15.0)));
				}
